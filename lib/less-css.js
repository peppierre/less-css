...
var clearComments, sortSelectors, purgeSpaceFromRule, generateRuleDescriptor, optimizeProperties, optimizeCssTree, generateCSSTree;
clearComments = function (css) {
    ...
    innerCss = innerCss.replace(new RegExp("\\/\\/.*", "gi"), "");
...
purgeSpaceFromRule = function (descriptor) {
...
generateRuleDescriptor = function (rule) {
    ...
    return purgeSpaceFromRule(descriptor);
};
...
generateCSSTree = function (css) {
    ...
    while (idxCloseCurlyBracket >= 0) {
        nextRule = innerCss.substring(0, idxCloseCurlyBracket + 1).replace(/^\s*/gi, "");
